...
public class BankingService
{
    ...
    public Result<Account> CreateAccount(string ownerName, Money initialDeposit, Guid idempotencyKey)
    {
        return _idempotency.ExecuteOnce(idempotencyKey, () =>
        {
            if (string.IsNullOrWhiteSpace(ownerName))
                return _auditor.Fail<Account>(Operation.CreateAccount, [], initialDeposit, idempotencyKey, "Owner name must not be empty.");

            if (initialDeposit.Amount <= 0)
                return _auditor.Fail<Account>(Operation.CreateAccount, [], initialDeposit, idempotencyKey, "Initial deposit must be positive.");
    ...
    public Result<LedgerEntry> Deposit(Guid accountId, Money amount, Guid idempotencyKey)
    {
        ...
        {
            if (amount.Amount <= 0)
                return _auditor.Fail<LedgerEntry>(Operation.Deposit, [accountId], amount, idempotencyKey, "Deposit amount must be positive.");

            var account = _accounts.GetById(accountId);
            if (account is null)
                return _auditor.Fail<LedgerEntry>(Operation.Deposit, [accountId], amount, idempotencyKey, "Account not found.");

            if (amount.Currency != account.Currency)
                return _auditor.Fail<LedgerEntry>(Operation.Deposit, [accountId], amount, idempotencyKey, $"Currency mismatch: account is {account.Currency}, deposit is {amount.Currency}.");
    ...
    public Result<LedgerEntry> Withdraw(Guid accountId, Money amount, Guid idempotencyKey)
    {
        ...
        {
            if (amount.Amount <= 0)
                return _auditor.Fail<LedgerEntry>(Operation.Withdraw, [accountId], amount, idempotencyKey, "Withdrawal amount must be positive.");

            var account = _accounts.GetById(accountId);
            if (account is null)
                return _auditor.Fail<LedgerEntry>(Operation.Withdraw, [accountId], amount, idempotencyKey, "Account not found.");

            if (amount.Currency != account.Currency)
                return _auditor.Fail<LedgerEntry>(Operation.Withdraw, [accountId], amount, idempotencyKey, $"Currency mismatch: account is {account.Currency}, withdrawal is {amount.Currency}.");

            lock (_locks.GetOrCreate(accountId))
            {
                var balance = ComputeBalance(accountId);
                if (balance < amount.Amount)
                    return _auditor.Fail<LedgerEntry>(Operation.Withdraw, [accountId], amount, idempotencyKey, "Insufficient funds.");
    ...
    public Result Transfer(Guid fromAccountId, Guid toAccountId, Money amount, Guid idempotencyKey)
    {
        ...
        {
            if (amount.Amount <= 0)
                return _auditor.Fail(Operation.Transfer, [fromAccountId, toAccountId], amount, idempotencyKey, "Transfer amount must be positive.");

            if (fromAccountId == toAccountId)
                return _auditor.Fail(Operation.Transfer, [fromAccountId], amount, idempotencyKey, "Cannot transfer to the same account.");

            var from = _accounts.GetById(fromAccountId);
            if (from is null)
                return _auditor.Fail(Operation.Transfer, [fromAccountId, toAccountId], amount, idempotencyKey, "Source account not found.");

            var to = _accounts.GetById(toAccountId);
            if (to is null)
                return _auditor.Fail(Operation.Transfer, [fromAccountId, toAccountId], amount, idempotencyKey, "Destination account not found.");

            if (amount.Currency != from.Currency)
                return _auditor.Fail(Operation.Transfer, [fromAccountId, toAccountId], amount, idempotencyKey, $"Currency mismatch: source account is {from.Currency}, transfer is {amount.Currency}.");

            if (amount.Currency != to.Currency)
                return _auditor.Fail(Operation.Transfer, [fromAccountId, toAccountId], amount, idempotencyKey, $"Currency mismatch: destination account is {to.Currency}, transfer is {amount.Currency}.");
            ...
            lock (_locks.GetOrCreate(secondId))
            {
                ...
                if (balance < amount.Amount)
                    return _auditor.Fail(Operation.Transfer, [fromAccountId, toAccountId], amount, idempotencyKey, "Insufficient funds.");
                ...
                catch
                {
                    ...
                    return _auditor.Fail(Operation.Transfer, [fromAccountId, toAccountId], amount, idempotencyKey, "Transfer failed after debit — compensated.");
                }
